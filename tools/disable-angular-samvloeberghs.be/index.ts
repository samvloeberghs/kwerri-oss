...
const disableAngularForGivenRoute = (html: string, assetsList: string[]): string => {
...
const injectSpeculationRules = (html: string): string => {
  ...
  if (speculationRules.prerender[0].urls.length) {
...
htmlFiles.forEach(async (file: string) => {
  ...
  const disabledHtmlWithSpeculationRules = injectSpeculationRules(disabledHtml);
  const minifiedHtml = await minifyHtml(disabledHtmlWithSpeculationRules);
